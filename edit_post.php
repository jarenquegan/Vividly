...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['editEntry'])) {
        ...
        if ($_FILES['artwork']['size'] > 0) {
            ...
            $stmt = $conn->prepare("UPDATE artworks SET title = ?, description = ?, style = ?, image_url = ? WHERE artwork_id = ?");
            $stmt->execute([$title, $description, $style, $artwork_filename, $artworkId]);
        } else {
            // User did not upload a new artwork, retain the existing image URL
            $stmt = $conn->prepare("UPDATE artworks SET title = ?, description = ?, style = ? WHERE artwork_id = ?");
            $stmt->execute([$title, $description, $style, $artworkId]);
        }

        // Update 'artists_artworks' table
        $stmt = $conn->prepare("UPDATE artists_artworks SET artist_id = ? WHERE artwork_id = ?");
    ...
    <section class="creator container" id="creator">
        <div class="creator-content">
            ...
            <form action="" method="POST" enctype="multipart/form-data" class="profile-form feedback-form" id="feedback-form">
                ...
                <input type="text" id="title" name="title" placeholder="Title" value="<?= isset($artwork['title']) ? htmlspecialchars($artwork['title']) : ''; ?>" required>

                <label for="description">Description</label>
                <textarea id="description" name="description" placeholder="Description" rows="10" style="resize: none;"><?= isset($artwork['description']) ? htmlspecialchars($artwork['description']) : ''; ?></textarea>

                <label for="style">Style</label>
                <input type="text" id="style" name="style" placeholder="Style" value="<?= isset($artwork['style']) ? htmlspecialchars($artwork['style']) : ''; ?>">
                <label for="style" class="note">Note: Use comma(,) as separator.</label>

                <label for="artwork">Artwork</label>
                <input type="file" id="artwork" name="artwork" class="avatar">
                <label for="artwork" class="note">Note: For a better viewing experience, use a 4:3 aspect ratio photo.</label>

                <div class="artwork-image">
                    <img id="previewImage" src="artworks/<?= $artwork['image_url'] ?>" loading="lazy" alt="Preview Image">
                </div>
                ...
                <input type="submit" id="submitButton" value="EDIT POST" name="editEntry" class="send-btn">
            </form>
