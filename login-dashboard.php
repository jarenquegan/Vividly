...
if (isset($_GET['username']) && $_GET['password']) {
    ...
    // Query the database to check if the user exists and has an account type of 'Admin'
    ...
    if ($artist) {
        if ($artist['acct_type'] === 'Admin') {
            ...
            $_SESSION['acct_type'] = $artist['acct_type'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($artist) {
        if ($artist['acct_type'] === 'Admin') {
            ...
            // User exists but does not have admin privilege
            $error = "You do not have admin privileges. Please log in as an admin.";
        }
    } else {
        // User not found
...
<body>
    ...
    <section class="login1 container" id="login1">
        ...
        <section class="login container" id="login">
            <!-- Heading -->
            <div class="heading-2">
                <h2 class="heading-title-2">ADMIN LOGIN</h2>
            </div>
            <!-- Login Form -->
        ...
        <section class="loginalt container" id="loginalt">
            <div>
                <p style="text-align: center;">
                    Wish to have an admin privileges?<br>Please contact queganjaren@gmail.com.
                </p>
            </div>

            <!-- Loginalt Links -->
            <div class="loginalt-links">
                <a href="login.php">Login to <?= $brand_name; ?> instead?</a>
            </div>
            <!-- Copyright -->
            <p style="text-align: center;">
                <span href="more.php" style="text-decoration: none; color: inherit;"><?= $brand_name; ?> &#169; 2023</span>
            </p>
        </section>
    </section>
    <!-- Email Js Link -->
