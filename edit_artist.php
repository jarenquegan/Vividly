...
if (isset($_GET['artist_id'])) {
    ...
    $sql = "SELECT * FROM artists WHERE artist_id = :artist_id";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':artist_id', $artist_idToEdit, PDO::PARAM_STR);
    $stmt->execute();
    $artistToEdit = $stmt->fetch(PDO::FETCH_ASSOC);

    // Assign the profile data to variables
    $username = $artistToEdit['username'];
    $firstname = $artistToEdit['firstname'];
    $middlename = $artistToEdit['middlename'];
    $lastname = $artistToEdit['lastname'];
    $suffix = $artistToEdit['suffix'];
    $email = $artistToEdit['emailaddress'];
    $bio = $artistToEdit['bio'];
    $artist_pic = $artistToEdit['artist_pic'];
    $password = $artistToEdit['password'];
    $acctType = $artistToEdit['acct_type'];
    $birthdate = $artistToEdit['birthdate'];
    $address = $artistToEdit['address'];
    $phone_number = $artistToEdit['phone_number'];
...
<body>
    ...
    <section class="creator container" id="creator">
        <div class="creator-content">
            ...
            <div class="profile-text">
                <?php
                if (!empty($artistToEdit['firstname']) || !empty($artistToEdit['middlename']) || !empty($artistToEdit['lastname']) || !empty($artistToEdit['suffix'])) {
                    $middleInitial = !empty($artistToEdit['middlename']) ? strtoupper($artistToEdit['middlename'][0]) . "." : "";
                    echo "<h2 id=\"artistName\">" . $artistToEdit['firstname'] . " " . $middleInitial . " <span class='color'>" . $artistToEdit['lastname'] . "</span>";

                    if (!empty($artistToEdit['suffix'])) {
                        echo ", " . $artistToEdit['suffix'] . "</h2>";
                    }
                } else {
                    echo "<h2>" . $artistToEdit['username'] . "</h2>";
                }
                ?>
            ...
            <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>?artist_id=<?= $artist_idToEdit; ?>" method="POST" enctype="multipart/form-data" class="profile-form feedback-form" id="feedback-form">
                ...
                <div class="option-container">
                    <input type="radio" id="Admin" name="acct_type" value="Admin" <?= isset($acctType) && $acctType === 'Admin' ? 'checked' : '' ?> required>
                    <label for="Admin" class="option-rad">Admin</label>

                    <input type="radio" id="User" name="acct_type" value="User" <?= isset($acctType) && $acctType === 'User' ? 'checked' : '' ?> required>
            ...
            <div class="logout" style="margin-bottom: 5rem !important;">
                <!-- <a href="delete_action.php?artist_id=<?= $artist_idToEdit; ?>&deleteInside=true"><i class='bx bx-trash'></i>
    ...
    <script>
        function updateArtistFName() {
            ...
            artistName.innerHTML = (firstName || "<?php echo $artistToEdit['firstname']; ?>") + " " + middleInitial + " <span class='color'>" + (lastName || "<?php echo $artistToEdit['lastname']; ?>") + "</span>";
